...
architecture behave of cpu is
    ...
    signal s_if_instr : std_logic_vector(31 downto 0) := (others => '0'); -- Hold the instruction until next clock cycle

    -- Decoder Signals
    signal s_dec_alu_instr : T_ALU_OP := ALU_ADD;
    signal s_dec_alu_flag  : std_logic := '0';
    ...
    signal s_ex_reg_wr_en : std_logic := '0';  -- Register write enable
    ...
    signal s_ex_pc_target : std_logic_vector(31 downto 0) := (others => '0'); -- Registered branch/jump target
    signal s_ex_pc_load_en : std_logic := '0'; -- Registered PC load enable
    ...
    begin
        case i is
            when INSTR_ADD|INSTR_SUB|INSTR_SLL|INSTR_SLT|INSTR_SLTU|INSTR_XOR|INSTR_SRL|INSTR_SRA|INSTR_OR|INSTR_AND|
                 ...
                 INSTR_LW|INSTR_LUI|INSTR_AUIPC =>
                return '1';
            when others =>
                return '0';
        end case;
...
begin
    ...
    pc_inst : entity work.pc
        ...
        i_rst => i_rst,
        i_en => '1', 
        i_load => s_ex_pc_load_en,
        i_load_addr => s_ex_pc_target,
        o_pc => s_pc,
        o_pc_next => s_pc_next);
    ...
    process(i_clk) -- set hold signal for instruction
    begin
    ...
    process(s_dec_instr, s_flag_z) -- Branch/jump taken decision (for now only supports BEQ, BNE and JAL)
    begin
        if s_dec_instr = INSTR_JAL then
            s_branch_taken <= '1';
    ...
    s_pc_target <= std_logic_vector(unsigned(s_pc) + unsigned(s_dec_imm));
    ...
    begin
        if rising_edge(i_clk) then
            if i_rst = '1' then
                ...
                s_ex_pc_target <= s_pc_target;
                s_ex_pc_load_en <= s_branch_taken;
                --, s_flag_gt, s_flag_gtu
            end if;
        end if;
    end process;
